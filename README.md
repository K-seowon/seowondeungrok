...
- `datasets/node_texts.csv` : 문건 원문을 분리한 텍스트 파일입니다.
- `ontology/seowondeungrok_ontology.ttl` : 『서원등록』 온톨로지 설계 파일
- `analysis/queries/` : Apache Jena Fuseki에서 실행한 SPARQL 질의문을 질의별 txt 파일로 정리한 폴더입니다.
...
- 문건 원문 확인: `node_texts.csv`를 사용하면 문건 원문을 별도로 확인할 수 있습니다.
